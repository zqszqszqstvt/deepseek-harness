...
Agent runtimes are a deployment contract. Strict confinement exposes read-only runtime directories, a minimal synthetic `/etc`, and the calling project's workspace while omitting `/run`, `/home`, `/opt`, `/var`, `/srv`, host accounts, and host Git/npm configuration. Shared Python and Node live under `/usr/local`; per-project packages stay in `.venv` and `node_modules`. Project creation also creates writable `.home` and `.cache` directories. Cloud subprocesses reject ambient Server environment inheritance, and the shell supplies a fixed PATH, project HOME, project caches, locale, and managed `DSH_*` facts. The deployment runner places each shell in a pasta network namespace whose nftables policy permits public egress but blocks host loopback, private/shared/link-local ranges, cloud metadata, and multicast; failure to create the namespace or policy fails the command closed. Cloud background jobs have a configurable lifetime and default to 30 minutes. `$DSH_HOME/AGENTS.md`, `$DSH_HOME/skills`, and `$DSH_AGENTS_HOME/skills` remain outside workspace-fenced reads; supported instruction delivery uses the trusted bundled skill root and deployment persona. [`deploy/dsh-server`](../../../deploy/dsh-server/README.md) owns the host preparation, security prerequisites, patch, and acceptance runbook.
...
The bundle exposes `switch_execution_environment` and an environment-neutral `shell` tool. The `shell` description tells the model to use the active environment's reported dialect; cloud commands may use lifetime-bounded background jobs, while local commands are foreground-only.
