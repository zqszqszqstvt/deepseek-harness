...
[`@deepseek-ai/dsh-subprocess`](../subprocess/README.zh.md) seam 的本地 Service Provider。`LocalSubprocessRuntime` 解析本地可执行文件，以显式 stdio spawn 普通 detached 进程树，并通过 `node-pty` 加平台进程检查实现终端进程。每项处置方式、限制、终端尺寸、宽限期与目录都来自调用方能力 seam（[`dsh-bash-local`](../../shell/bash-local/README.zh.md)、[`dsh-lsp-stdio`](../../lsp/lsp-stdio/README.zh.md) 和 [`dsh-terminal-bash`](../../terminal/terminal-bash/README.zh.md)）；provider 配置只控制子进程环境基底。

## 配置

`inheritParentEnv` 默认为 `true`，保留经过凭据及 `DSH_*` 清除的父进程基底。设为 `false` 后，可执行文件查找、普通进程和终端都从空环境开始。`baseEnv` 在每次调用的显式 `env` 之前提供部署自有条目，调用参数仍是最终覆盖层。Server 云端 provider 使用 `inheritParentEnv: false` 与固定 PATH，因此服务凭据、代理变量、home 和 loader hook 不会作为环境变量隐式进入模型命令。
...
- **按流划分的处置方式**：`'pipe'` 把原始流原样交给调用方（协议分帧仍归消费方所有）；`'inherit'` 直通父进程的描述符；收集模式（collect）在输出超过上限后于内存中保留尾部（错误与结果通常聚集在末尾，沿用 pi/OpenCode 的理由），并在配置了 spill 上限时把完整流追加到一个私有临时文件；省略 `spill` 则只保留用于诊断的尾部。某条流大于 spill 上限时，会丢弃已不完整的 spill，仅返回带截断标记的尾部；spill 文件描述符在结算时封存，最终关闭失败时则不公布路径，以免声称存在不完整的文件。spill 文件权限为 `0600`、名称随机，位于按需创建、权限为 `0700` 的每进程目录之下；当调用方在 spec 上给出 `spillDir` 时则位于该目录之下——被限制在单一工作区内的会话会传入其内部的目录，因此所报告的 `spillPath` 对该会话确实可以重新打开。该目录按需创建；无法打开或写入的 spill 会被丢弃，只保留有界尾部，而不会从流监听器中逃逸为宿主错误（见[工作区内 spill 产物 Agent Note](../../../.agents/notes/implemented/feature/2026-09-09-workspace-placed-spill-artifacts.zh.md)）。
- **配置基底 + 显式合并**：启用继承时，基底是移除凭据形态变量（`*KEY*`／`*PASSWORD*`／`*SECRET*`／`*TOKEN*`）和所有环境 `DSH_*` 名称后的 `process.env`；禁用继承时基底为空。随后合并 `baseEnv`，最后合并 spec 的显式 `env` 且不做 namespace 校验，因此有意给出的条目会覆盖，而部署可以排除全部环境变量。提供的 stdin 会被写入后关闭；否则 fd 0 指向 `/dev/null`。参见 [stdin/env Agent Note](../../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.zh.md)与[受管环境 Agent Note](../../../.agents/notes/implemented/feature/2026-07-10-agent-session-identity-and-log-location.zh.md)。
