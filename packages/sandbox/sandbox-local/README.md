...
The bwrap profile combines a read-only host root, fresh `/dev`, and private-PID `/proc`. Commands manage descendants but cannot see host processes; hiding host `/proc/<pid>` entries prevents magic links such as `root` and `fd` from bypassing its mounts. `workspace-write` adds ephemeral `/tmp` and a writable workspace bind. The Server's `strictFilesystem` profile starts from an empty root, binds runtime directories and the workspace, constructs a minimal `/etc` from optional read-only runtime files, omits `/run`, and unshares PID, IPC, UTS, and cgroup namespaces before remounting the root read-only. Network isolation is not implied by `strictFilesystem`; a deployment that needs it supplies a `runnerCommand`, such as the pasta+nftables prefix in [`deploy/dsh-server`](../../../deploy/dsh-server/README.md). The [private-PID Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-06-bwrap-private-pid-namespace.md) records the process boundary.
