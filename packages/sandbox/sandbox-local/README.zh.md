...
bwrap profile 将只读宿主根目录、全新的 `/dev` 与使用私有 PID namespace 的 `/proc` 组合起来。命令可管理后代进程，但看不到宿主进程；隐藏宿主的 `/proc/<pid>` 条目，可以防止 `root`、`fd` 等魔法链接绕过其挂载约束。`workspace-write` 另加临时 `/tmp` 与可写工作区绑定挂载。Server 的 `strictFilesystem` profile 从空根目录开始，绑定运行时目录与工作区，用可选只读运行时文件构造最小 `/etc`，省略 `/run`，并隔离 PID、IPC、UTS 与 cgroup namespace，最后把根重新挂成只读。`strictFilesystem` 本身不承诺网络隔离；有此需求的部署需要提供 `runnerCommand`，例如 [`deploy/dsh-server`](../../../deploy/dsh-server/README.zh.md) 中的 pasta+nftables 前缀。[私有 PID Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-06-bwrap-private-pid-namespace.zh.md)记录该进程边界。
