...
`resolveConfinedCwd(requested, policy)` is the shared process-working-directory guard. After a consumer resolves deployment caps and one-shot approval, the helper canonicalizes relative paths against `workspaceRoot`; `read-only` and `workspace-write` reject outside absolute paths and parent, drive, or symlink escapes, while `danger-full-access` preserves an explicit cwd. Model-facing shell tools call it before executor resolution or background-job publication, and the local terminal backend calls it before PTY allocation. It governs the process's model-selected initial cwd, not later `chdir` calls inside command text; the sandbox backend remains responsible for actual file visibility and effects after spawn.
