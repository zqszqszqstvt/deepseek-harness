...
| `workdir` | string | Working directory for this call. Defaults to the filesystem identity of the calling agent's session cwd (`session.header.cwd`) so each session runs in its own workspace; a relative `workdir` is resolved against that same identity. Under `read-only` or `workspace-write`, the canonical result must remain inside the session workspace. |
| `run_in_background` | boolean | Return a job id immediately; no timeout applies. |
| `sandbox_permissions` | string enum | ADVERTISED ONLY when the mounted executor sandboxes and `ctx.sandboxPolicy.escalationTargets` is non-empty: one deployment-approved wider mode for a denied command. Strict widening is checked at execution against the per-session effective mode, and a disabled or non-widening request fails without prompting anyone. |
...
`command`, `workdir`, and `timeoutMs` are resolved against the executor's config defaults via `ctx.shell.resolve()` before execution, so the Service Definition (`ShellExecSpec`) receives explicit `workdir`/`timeoutMs` values. The workdir default is applied in the tool layer from the calling agent's `session.header.cwd` BEFORE `resolve()` — the per-session cwd must come from `exec.agent`, since N sessions share one executor; only when no session cwd is available does the executor fall back to its own config / `process.cwd()`. When sandbox policy is present, the tool reuses its already-canonical `workspaceRoot` as the workdir base. After any one-shot approval supplies the final mode, the shared sandbox cwd guard rejects a confined path outside that root before `ctx.shell.resolve()`, `ctx.jobs.start()`, or process creation. An approved `danger-full-access` call and every non-sandbox composition retain external absolute-workdir support.
...
Escalating bash calls first require the requested mode in `ctx.sandboxPolicy.escalationTargets`, then resolve `ctx.approval` before execution. `allowed-once` applies the requested mode only to that call; deployment denial, rejection, cancellation, unavailability, or missing approval context executes nothing and returns a distinct error. On a real denial, the model may retry the same command once in the same turn with the narrowest sufficient mode and justification; the approval prompt itself is the consent step. Escalation is never speculative, and a disabled or rejected approval is final. The [sandbox Agent Note](../../../.agents/notes/implemented/feature/2026-07-06-sandbox.md) owns the general rationale; the [deployment ceiling decision](../../../.agents/notes/implemented/bug-fix/2026-08-27-server-sandbox-maximum.md) owns the Server restriction.

## Per-session mode switching

For sandboxing executors, each call resolves mode as an allowed one-shot escalation, then session override, then deployment default, with `maximumMode` capping every source. Non-sandboxing and agent-less calls carry no session override. The policy owner contributes the current capability-neutral standing mode; denial results include retry guidance only when the deployment advertises an escalation target. See the [`dsh-shell` fold](../shell/README.md) and [sandbox switching contract](../../../.agents/notes/implemented/feature/2026-07-06-sandbox.md).
...
The model sees the generated [`bash` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-bash). `run_in_background` appears only when this producer enables it; `sandbox_permissions` and `justification` appear only when the mounted executor advertises sandboxing and deployment policy permits at least one escalation target. Agent-scoped tool restrictions can remove the definition for that agent.
...
Validation and policy failures are normalized as `Error: <message>`. This package's stable messages are `invalid command: expected a non-empty string`, `invalid description: expected a non-empty string`, `invalid timeoutMs: expected a positive number, got <value>`, `invalid escalation: sandbox_permissions requires a justification`, `invalid escalation: justification is only valid together with sandbox_permissions`, `invalid justification: expected a non-empty sentence`, `background execution is disabled for this bash tool`, `background jobs unavailable: load @deepseek-ai/dsh-jobs and @deepseek-ai/dsh-tool-jobs`, `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)`, `sandbox escalation is disabled by deployment policy`, `sandbox escalation to "<mode>" is disabled by deployment policy`, `sandbox escalation to "<mode>" is not strictly wider than this call's current "<mode>" mode`, the approval-availability/rejection/cancellation variants, and `tool call aborted`.
