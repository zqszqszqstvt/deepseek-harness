...
- **Spill placement follows the session's read boundary** — with `spillPlacement: session-workspace`, a truncated command's full-stream spill file lands under `<workspace>/.dsh/spill` — the resolved sandbox-policy root, else the command's own workdir — instead of the subprocess service's host-private directory, so the `spillPath` this executor reports is one a session confined to that workspace can reopen. A `read-only` policy keeps the private directory: that mode promises no workspace writes, and this artifact is the harness's own, not the confined child's. An unwritable spill directory degrades to the bounded in-memory tail instead of failing the run ([workspace-placed spill artifacts Agent Note](../../../.agents/notes/implemented/feature/2026-09-09-workspace-placed-spill-artifacts.md)).
