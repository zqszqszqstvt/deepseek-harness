...
`relocateStoredSessionCwd(id, cwd)` is a deployment-adapter operation for moving one cold session within the configured root while preserving its id and event log. The caller must exclusively own and authorize the relocation. It rejects a live session or occupied destination, replaces the stored header before moving the session directory, and can complete that move when retried after an interruption.
