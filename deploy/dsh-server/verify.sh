...
echo
echo "== 7. end-to-end against a live server: see ACCEPTANCE.md (7 turns, mechanical pass criteria) =="
cat <<'EOF'
  PORT=3081; U1=acca; U2=accb; P=acc
  B1="127.0.0.1:$PORT/v1/users/$U1/projects/$P"
  curl -s -X PUT "$B1/session" | head -c 200; echo
  # Then follow ACCEPTANCE.md phases 1-8. Post payloads from files generated by
  # python3 (--data-binary @file): a hand-typed multi-line JSON body picks up a
  # real newline from terminal wrapping, the body parse fails, and the client
  # sees only the generic "server request failed".
  # Judge each turn with:
  #   ./show-history.py --user $U1 --project $P --port $PORT --grep <string>
EOF
