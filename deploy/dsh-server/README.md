...
| `/usr/local/bin/node`, `npm`, `npx`, `pnpm` | shared Node.js LTS and its package managers | read-only | `root:root`, installed before the lock |
| `/etc/pip.conf`, `/etc/uv/uv.toml` | optional index config; absent means the public index | read-only | `root:root 0644`, no credentials |
| `/usr/local/share/dsh/skills/runtime-env/SKILL.md` | model-facing command templates | not needed: read host-side | `root:root`, `a-w` |
