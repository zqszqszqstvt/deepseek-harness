...
A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@deepseek-ai/dsh-sandbox-policy`](../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. After that final mode is known, the model-facing Bash and PowerShell consumers apply the sandbox seam's shared cwd guard before executor resolution or background-job publication: confined `workdir` values must canonically remain inside the session workspace, while approved `danger-full-access` and non-sandbox calls retain external directory support. The mode/root/enforcement vocabulary is owned by the [`@deepseek-ai/dsh-sandbox` seam](sandbox.md); modes govern file effects only, so command-internal `chdir` remains subject to the backend's actual file visibility and effect restrictions rather than this initial-directory guard.
