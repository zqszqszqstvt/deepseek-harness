...
| `ctx.cloudExecution` | `core` | [`server`](../packages/bundle/server) | - | [`server`](../packages/bundle/server) | - | Captures the Server cloud filesystem, subprocess, and shell Providers inside an isolated realm so the root runtime routers can dispatch without exposing or replacing them. |
| `ctx.executorBroker` | `core` | [`server`](../packages/bundle/server) | - | [`server`](../packages/bundle/server) | - | Owns authenticated-by-routing executor connections, correlates bounded local operations, and rejects responses whose binding or environment epoch no longer matches the request. |
| `ctx.serverEnvironments` | `core` | [`server`](../packages/bundle/server) | - | [`server`](../packages/bundle/server) | - | Owns durable active bindings, process-local executor availability, and execution leases for deterministic Server project Sessions. |
| `ctx.serverRuntimeRouter` | `core` | [`server`](../packages/bundle/server) | - | [`server`](../packages/bundle/server) | - | Resolves the initiating Agent Session to one binding and epoch, then dispatches through the exact cloud Provider or local executor without fallback. |
| `ctx.serverStartup` | `core` | [`server`](../packages/bundle/server) | - | [`server`](../packages/bundle/server) | - | Publishes validated command-line deployment values before the Server HTTP and persistence plugins are composed. |
